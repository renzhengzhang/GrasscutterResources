...
function action_timer_event(context, evt)
	...
	if evt.source_name == "SwitchToInvisible" then
		suites_index = ScriptLib.GetGroupVariableValue(context, "suites_index")
		gadget_list = suites[defs_floor_groups[suites_index].invisible_suites].gadgets
...
function LF_Create_Floor_Group(context)
	--随机选择一条路线
	randomIdx = math.random(1,#defs_floor_groups)
