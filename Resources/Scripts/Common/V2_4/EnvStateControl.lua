...
function action_select_option_control(context,evt)
    ...
    is_in_cd =  ScriptLib.IsLevelTagChangeInCD(context, 1)
...
function action_time_axis_pass(context,evt)
    if (evt.source_name == "level_tag_change_delay") then
        ...
        env_state =  3-current_env_state_id
...
function LF_Set_Option(context,controller,option_id)
    state = ScriptLib.GetGadgetStateByConfigId(context,0,controller)
    if (state == 202 or state == 302 or state == 212 or state == 312) then
        ret = ScriptLib.SetWorktopOptionsByGroupId(context,0,controller,{option_id})
    end
end
