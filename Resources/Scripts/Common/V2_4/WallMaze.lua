...
function action_group_load(context,evt)
    ...
    for i = 1, #StartWallMap do
        wall = StartWallMap[i][1]
        index = StartWallMap[i][2]
        ScriptLib.PrintContextLog(context,"WallMaze: group加载，设置墙 "..wall.." 位置： "..index)
        LF_Set_Block_Point(context,wall,index,1)
    end
    for i = 1, #StartBlockerMap do
        blocker = StartBlockerMap[i][1]
        index = StartBlockerMap[i][2]
        block_state = StartBlockerMap[i][3]
        state = ScriptLib.GetGadgetStateByConfigId(context, 0, blocker)
...
function action_gadget_state_change(context,evt)
    ...
    controller = evt.param2
    gadget_state = evt.param1
    is_moving = ScriptLib.GetGroupVariableValue(context,"is_moving")
    --没有墙处于移动状态
    if (is_moving ~= 1) then
        --触发的是控制器
        if (LF_Is_Controller(context,controller)) then
            if (gadget_state == 222 or gadget_state == 322) then
                ...
                wall_list = LF_Get_Controller_Wall(context,controller)
                block_table = LF_Get_Block_Table(context)
                block_table = LF_Pre_Move(context,block_table,wall_list)
                for i = 1, #wall_list do
                    path_state = LF_Get_Path_State(context,block_table,controller,wall_list[i])
                    next_point = LF_Get_Next_Point(context,path_state)
...
function LF_Check_Success(context)
    ...
    for i = 1, #StartWallMap do
        wall = StartWallMap[i][1]
        wall_pos = LF_Get_Wall_Pos(context,wall)
...
function LF_Is_In_Block_State(context,blocker_id)
    state = ScriptLib.GetGadgetStateByConfigId(context,0,blocker_id)
    blocker_info = LF_Get_Blocker_Info(context,blocker_id)
...
function LF_Get_Wall_Pos(context,wall_id)
    wall_index = LF_Get_Wall_Index(context,wall_id)
    wall_pos = ScriptLib.GetGroupVariableValue(context,"wall_"..wall_index)
    return wall_pos
end

--输入一个speicla blocker Id，返回其应当阻塞的位置
function LF_Get_Speical_Blocker_Pos_List(context,blocker_id)
    blocker_index = LF_Get_Sepcial_Blocker_Index(context,blocker_id)
    pos_list = {}

    if (SpecialBlockerMap ~= nil) then
        for i = 1, #SpecialBlockerMap[blocker_index][2] do
            pos = ScriptLib.GetGroupVariableValue(context,"special_blocker_"..blocker_index.."_"..i)
...
function LF_Get_Controller_Wall(context,controller_id)
    controller_info = LF_Get_Controller_Info(context,controller_id)
    walls = {}
...
function LF_Set_Block_Point(context,blocker,point,type)
    --墙
    if (type == 1) then
        blocker_index = LF_Get_Wall_Index(context,blocker)
    ...
    --阻塞物
    if (type == 2) then
        blocker_index = LF_Get_Blocker_Index(context,blocker)
    ...
    if (type == 3) then
        blocker_index = LF_Get_Special_Blocker_Index(context,blocker)
        for i = 1,#point do
...
function LF_Can_Block_State_Change(context,blocker,target_gadget_state)
    blocker_info = LF_Get_Blocker_Info(context,blocker)
    block_pos = LF_Get_Blocker_Pos(context,blocker)
    is_blocked = LF_Is_Blocked(context,block_pos)
...
function LF_Set_Block_After_State_Change(context,blocker,target_gadge_state)
    if (LF_Is_Blocker(context,blocker)) then
        ...
        if (blocker_info[3] == target_gadge_state) then
            ...
            pos = LF_Get_Blocker_Pos(context,blocker)
...
function LF_Get_Block_Table(context)
    ...
    for i = 1,#StartWallMap do
        wall_pos = LF_Get_Wall_Pos(context,StartWallMap[i][1])
        Block_Table[wall_pos] = 1
    end
    for i = 1,#StartBlockerMap do
        blocker_pos = LF_Get_Blocker_Pos(context,StartBlockerMap[i][1])
    ...
    if (SpecialBlockerMap ~= nil) then
        for i = 1,#SpecialBlockerMap do
            special_block_pos_list = LF_Get_Speical_Blocker_Pos_List(context,SpecialBlockerMap[i])
...
function LF_Pre_Move(context,block_table,wall_list)
    ...
    pre_move_block_table = block_table
    for i = 1, #wall_list do
        wall_pos = LF_Get_Wall_Pos(context,wall_list[i])
...
function LF_Get_Path_State(context,block_table,controller,wall)
    ...
    path = LF_Get_Controller_Wall_Path(context,controller,wall)
    path_state = {}
    wall_pos = LF_Get_Wall_Pos(context,wall)
    index = LF_Get_Index_In_Table(context,path,wall_pos)
