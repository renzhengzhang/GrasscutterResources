...
pos = regions[defs.related_big_region_config_id].pos
markGadget = 	{ config_id = 80001, gadget_id = 70220006, pos = { x = pos.x, y = pos.y, z = pos.z }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
...
function action_EVENT_ENTER_REGION(context, evt)
	...
	if evt.param1 == defs.target_region_config_id then
        fix_lod_level = ScriptLib.GetGroupVariableValue(context, "lodLevel")
        temp = ScriptLib.SetPlayerEyePointLOD(context, defs.target_region_config_id, defs.related_big_region_config_id, fix_lod_level)
...
function action_EVENT_LEAVE_REGION(context, evt)
	...
	if evt.param1 == defs.target_region_config_id then
        temp = ScriptLib.ClearPlayerEyePoint(context, defs.target_region_config_id)
