...
function action_monster_tide_die(context,evt)
	tide_index = ScriptLib.GetGroupTempValue(context, "Cur_Tide_Index", {})
    if evt.param1 >= #monster_list[tide_index].tide_mons then
    	ScriptLib.PrintContextLog(context,"##[LumenArena]:当前怪物潮怪物全部清空")
    	next_refresh_time = monster_list[tide_index].next_time
...
function action_monster_die_before_leave_scene(context, evt)
	mon_id = monsters[evt.param1].monster_id
...
--			ScriptLib.RemoveEntityByConfigId(context, base_info.group_id, EntityType.GADGET, mud_kv.mud_id)
			...
			--根据场上黑泥核心的状态移除包裹物
			cover_id = mud_kv.charge_point_mud_cover_id
			core_nums = 0
	...
	if 0 == ScriptLib.GetGroupTempValue(context, "Mud_Num", {}) then
		...
		roate_state = ScriptLib.GetGroupTempValue(context, "Pursina_Rotate", {})
...
function LF_CreateMonsterTide(context)
	...
	--刷新怪物潮
	cur_tide_index = ScriptLib.GetGroupTempValue(context, "Tide_Index", {})
	ScriptLib.SetGroupTempValue(context, "Cur_Tide_Index", cur_tide_index, {})
	cur_tide_num = ScriptLib.GetGroupTempValue(context, "Tide_Num", {})
	cur_mons_list = monster_list[cur_tide_index].tide_mons
	next_tide_index = monster_list[cur_tide_index].next_tide
...
function LF_CreateMud(context)
	...
	cur_tide_index = ScriptLib.GetGroupTempValue(context, "Tide_Index", {})
	time_refresh_delay = monster_list[cur_tide_index].tide_delay
	--创建黑泥
	cur_mud_list = monster_list[cur_tide_index].mud
	if #cur_mud_list ~= 0 then
		ScriptLib.PrintContextLog(context,"##[LumenArena]:补充黑泥")
		for i=1,#cur_mud_list do
			mud_index = cur_mud_list[i]
