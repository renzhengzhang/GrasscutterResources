...
function PrintLog(context, content)
	log = "## [LibraryElevator] TD: "..content
...
function LF_SetConsoleOptions(context,floor)
    ...
    --获取任务进度
    progress=ScriptLib.GetGroupVariableValue(context, "questUnlockProgress")
    consoleConfigId=floorInfo[floor].console
    --底层只上向上按钮
    if floor==1 then
...
function action_VARIABLE_CHANGE(context, evt)
    curFloor=ScriptLib.GetGroupVariableValue(context, "currentFloor")
...
function action_EVENT_SELECT_OPTION(context, evt)
    curFloor=ScriptLib.GetGroupVariableValue(context, "currentFloor")
    questStep=ScriptLib.GetGroupVariableValue(context, "questUnlockProgress")
    ...
    elseif evt.param2 ==globalInfo.summonOption then
        summonFloor=LF_FindSummonLevel(context,evt.param1)
