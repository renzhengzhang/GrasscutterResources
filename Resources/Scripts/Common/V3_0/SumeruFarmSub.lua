...
function LF_Initialize_Level()
    ...
    var = { config_id= 40010101, name = FarmPlay.ExhibitionVar, value = 0, no_refresh = true }
...
function action_CheckHarvest(context,evt)
    ...
    curFlowerIndex = QuestIndex[math.floor(evt.param1/100)]
...
function LF_CheckFlower(context)
    ...
    -- 查询bin，获取所有已种植list
    levelData = LF_GetCurLevelData(context)
    exhibitionData = LF_Exhibition_GetLevelData(context)
    if levelData ~= exhibitionData then

        exhibitionDataArray =  LF_DecToBin(exhibitionData)
        levelDataArray = LF_DecToBin(levelData)
        msg = "## TD_SubSumeruFarm : LF_CheckFlower 数据不一致"
        ...
        for k,v in pairs(exhibitionDataArray) do
            if v == 1 and levelDataArray[k] ~= 1 then
                ...
                msg = "## TD_SubSumeruFarm : LF_CheckFlower 正在修改Index"
                msg = msg .. " |index = " .. k
                ScriptLib.PrintContextLog(context, msg)
...
function LF_FlowerShow(context,index)
    -- 对应田切202
    cidlist = FlowerDic[index].cidList
...
function LF_CompleteCurLevel(context,index)
    levelData = LF_GetCurLevelData(context)
    changeLevelData = LF_SetTargetLevel(levelData,index,1)
...
function LF_FarmSubComplete(context)
    ...
    for k,v in pairs(levelDataArray) do
        if v == 0 then
            msg = "## TD_SubSumeruFarm : LF_FarmSubComplete 正在修改Index"
            msg = msg .. " |index = " .. k
            ScriptLib.PrintContextLog(context, msg)
            LF_FlowerShow(context,k)
        end
...
function LF_FarmProtect(context)
    ...
    for k,v in pairs(levelDataArray) do
        if v == 1 then
            msg = "## TD_SubSumeruFarm : LF_FarmProtect 保护需要设置的状态"
            msg = msg .. " |index = " .. k
            ScriptLib.PrintContextLog(context, msg)
            cidlist = FlowerDic[k].cidList
...
function LF_GetCurLevelData(context)
    levelData = ScriptLib.GetGroupVariableValueByGroup(context, FarmPlay.ExhibitionVar,0)
    return levelData
end
...
function LF_Exhibition_GetLevelData(context)
    ...
    levelDataDec = ScriptLib.GetGroupVariableValueByGroup(context, "FarmExhibition", 133301057)

    msg = "## TD_SubSumeruFarm : LF_Exhibition_GetLevelData"
