...
function LF_GetCloestPoint(context)
	targetpos={0,0,0}
	uid=ScriptLib.GetSceneUidList(context)[1]
	avatarid=GetAvatarEntityIdByUid(context, uid)
	avatarpos=ScriptLib.GetPosByEntityId(context,avatarid)
	shortest_key=0
	shortest_path_square=10000000
	for k,v in pairs(points) do
		pathlength_square=(v.pos.x-avatarpos.x)*(v.pos.x-avatarpos.x)+(v.pos.y-avatarpos.y)*(v.pos.y-avatarpos.y)+(v.pos.z-avatarpos.z)*(v.pos.z-avatarpos.z)
