...
function action_Group_Load(context, evt)
	state = ScriptLib.GetGroupVariableValue(context, "theatre_state")
...
function LF_SequenceAction(context)
	...
	if 0 ~= step.config_id then
		...
		if 0 ~= step.reminder_id then
			...
			pos = LF_GetEntityPosition(context, gadgets, step.config_id)
		...
		pos = LF_GetEntityPosition(context, regions, defs.play_region)
		if 0 ~= pos then
			ScriptLib.ShowReminderRadius(context, step.reminder_id, pos, reminder_radius)
		end
...
function action_TimeAxis_Pass(context, evt)
	theatre_state = ScriptLib.GetGroupVariableValue(context, "theatre_state")
