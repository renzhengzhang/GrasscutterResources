...
function LF_InitPosition(context)
	pattern_index = ScriptLib.GetGroupVariableValue(context, "pattern")
...
function action_Reach_Point(context, evt)
	...
	pattern_index = ScriptLib.GetGroupVariableValue(context, "pattern")
	--检查是否reach它最后一个point
	point_table = defs.patterns[pattern_index][evt.param1].point_id
	final_point = point_table[#point_table]
