...
function action_Tile_State_Change(context, evt)
	...
	starter_gadget_table = suites[defs.stages[stage_index][1]].gadgets
...
function LF_Check_AllPlayer_OutRegion(context, region_id)
	...
	eid = ScriptLib.GetEntityIdByConfigId(context, region_id)
...
function LF_LockAllGadgets(context, stage_index)
	...
	tile_gadget_table = suites[defs.stages[stage_index][2]].gadgets
...
function LF_TryShowGuide(context)
	...
	ret = ScriptLib.TryRecordActivityPushTips(context, 2014001)
	if 0 == ret then
