...
function action_Group_Load(context, evt)
	...
	play_state = ScriptLib.GetGroupVariableValue(context, "play_state")
	if 4 == play_state then
		return 0
	end
	stage = ScriptLib.GetGroupVariableValue(context, "cur_stage")
...
function LF_InitTheatre(context)
	...
	--uid = ScriptLib.GetSceneOwnerUid(context) --不处理SGV因为玩家正在玩其他Group会影响到,改到主控Group的EnterRegion
...
function LF_HandleTalkGadget(context)
	...
	if 2 == ScriptLib.GetHostQuestState(context,7902603) then
		ret = ScriptLib.SetGadgetTalkByConfigId(context,199002048,48008,0)
...
function LF_ResetVariables(context)
	...
	state = ScriptLib.GetGroupVariableValue(context, "play_state")
...
function SLC_NightCrowPickUp(context)
	...
	--添加“放置”选项
	stage = ScriptLib.GetGroupVariableValue(context, "cur_stage")
	stage_table = defs.question_suits[stage]
...
function LF_PlaceNightCrow(context, option_configID, gadget_configID)
	...
	ret = ScriptLib.CreateGadgetByConfigIdByPos(context, gadget_configID, gadgets[option_configID].pos, gadgets[option_configID].rot)
...
function LF_HandleSpecCrowAction(context, stage)
	step = defs.spec_actions[stage]
	--取实时位置
	pos = LF_GetEntityPosition(context, step.config_id)
...
function LF_AutoPickAndPlace(context)
	...
	config_id = defs.question_suits[stage].correct_gadget
	slot = defs.question_suits[stage].correct_slot
...
function LF_SequenceAction(context, type)
	...
	ret = ScriptLib.ChangeGroupTempValue(context, "action_index", 1, {})
	if -1 == ret then
	...
	stage_table = defs.actions[stage]

	--重置上一步演员状态
	if 1 < cur_index then
		last_actor = stage_table[cur_index - 1].config_id
	...
	if 0 ~= step.config_id then
		...
		pos = LF_GetEntityPosition(context, 0)
		if 0 ~= pos then
			ScriptLib.ShowReminderRadius(context, step.reminder_id, pos, cfg.reminder_radius)
		end
...
function LF_SetPickPhrase(context, is_fail)
	...
	if 1 == is_fail then
		config_id = ScriptLib.GetGroupTempValue(context, "placed", {})
...
function action_Place_NightCrow_Option(context, evt)
	...
	picked_up = ScriptLib.GetGroupTempValue(context, "picked_up", {})
...
function LF_ResetStagePlay(context)
	stage = ScriptLib.GetGroupVariableValue(context, "cur_stage")
	theatre_state = ScriptLib.GetGroupVariableValue(context, "play_state")
	cur_index = ScriptLib.GetGroupTempValue(context, "action_index", {})
...
function LF_GetEntityPosition(context, config_id)
	...
	entity_id = ScriptLib.GetEntityIdByConfigId(context, config_id)
	...
	pos_raw = ScriptLib.GetPosByEntityId(context, entity_id)
	...
	for k, v in pairs(defs.actor_list) do
		if k == stage then
			for ik, iv in pairs(v) do
				...
				if 199002043 == base_info.group_id then
					flag = ScriptLib.GetGroupVariableValue(context, "talk_done")
