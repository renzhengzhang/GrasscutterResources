...
function action_platform_arrival(context,evt)
    if LF_Is_Checkpoint(context,evt.param3) then
        ...
        current_point = LF_Get_Checkpoint_By_Route_Point(context,evt.param3)
...
function LF_Init_Timeback_Gadget_Config(context)
    ...
    for k,v in pairs(timeback_gadget_config) do
        ...
        if LF_Has_Ani_Timeback(context,k) then
	        ...
	        ret = ScriptLib.SetEntityServerGlobalValueByConfigId(context,k,"SGV_ani_"..local_defs.timeback_value_min_suffix, range[1])
        ...
        if LF_Has_Eff_Timeback(context,k) then
            eff_range = LF_Get_Effect_Timeback_Value_Config(context,k)
...
function LF_On_Rune_Leave_Worktop(context)
    worktop = LF_Get_Current_Worktop(context)
    ...
    current_point = LF_Get_Current_Point(context)
...
function LF_Get_Effect_Timeback_Value_Config(context,config_id)
    gadget_id = LF_Get_Gadget_Id_By_Config_Id(context, config_id)
    eff_range = local_timeback_gadget_config[gadget_id].eff_range
    ani_range = timeback_gadget_config[config_id].range
    is_revert = timeback_gadget_config[config_id].is_revert

    if is_revert then
        eff_range[1] = 1 - eff_range[2]
        eff_range[2] = 1 - eff_range[1]
    end
    actual_max = ani_range[1] + (ani_range[2] - ani_range[1]) * eff_range[2]
    actual_min = ani_range[1] + (ani_range[2] - ani_range[1]) * eff_range[1]
...
function LF_Start_Platform(context)
    current_checkpoint = ScriptLib.GetGroupVariableValue(context,"current_checkpoint")
    if current_checkpoint >= #checkpoints_to_route_points then
        return
    end
    target_point = current_checkpoint + 1

    start_array_point = 0
    if current_checkpoint > 0 then
        start_array_point = checkpoints_to_route_points[current_checkpoint]
    end
    target_array_point = checkpoints_to_route_points[target_point]
...
function LF_Set_Worktop_State(context,state)
    current_point = ScriptLib.GetGroupVariableValue(context,"current_checkpoint")
    current_worktop = checkpoint_to_worktops[current_point]
