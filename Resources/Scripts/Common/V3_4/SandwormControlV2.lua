...
function Initialize()
    ...
    current_offset = local_defs.array_base_offset
    for k,v in pairs(arrays) do
        array_name = k
        array_capacity = v.capacity
        for i = 1, array_capacity do
            var_id = current_offset + i
...
function LF_Request_Create_Sandworm_Params(context,prev_context,group_id,priority)
    ...
    ret = LF_Insert_Request_To_List(context,create_request)
    return ret
end
...
function LF_Request_Remove_Sandworm_Params(context,prev_context,destroy_request_id)
    ...
    index = LF_Get_Request_Index_From_List_By_Request_Id(context,destroy_request_id)
    if index == -1 then
        return -1
    end
...
function LF_Insert_Request_To_List(context,request)
    ...
    i = LF_Array_Get_First_Empty_I(context,request_struct.group_id)
    if i == -1 then
...
function LF_Print_Current_Request_List(context)
    ...
    for i = 1, local_defs.request_list_capacity do
        ...
        for k,v in pairs(arrays) do
            value = LF_Array_Get_I(context,k,i)
