...
function action_gadget_state_change(context,evt)
    ...
    if flag then
        ...
        is_success = true
        for i = 1, #JamshidRocks do
            state = ScriptLib.GetGadgetStateByConfigId(context,base_info.group_id,JamshidRocks[i])
...
function SLC_Rock_Being_Hit(context)
    for i = 1, #JamshidRocks do
        if context.source_entity_id == ScriptLib.GetEntityIdByConfigId(context, JamshidRocks[i]) then
            ...
            is_changeing = ScriptLib.GetGroupVariableValue(context,"is_changing")
            if is_changeing == 0 then
                ...
                state = ScriptLib.GetGadgetStateByConfigId(context,base_info.group_id,rock)
                current_answer = state_solution_defs[state]
                ScriptLib.PrintContextLog(context,"## [JamshidRock] SLC_Rock_Being_Hit: 当前答案为"..current_answer)
                next_answer = ((current_answer)%4)+1
                ScriptLib.PrintContextLog(context,"## [JamshidRock] SLC_Rock_Being_Hit: 下一个答案为"..next_answer)
                next_state = solution_state_defs[next_answer]
