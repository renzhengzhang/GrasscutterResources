...
function action_EVENT_SELECT_OPTION(context, evt)
	...
	cid = evt.param1//1000
	belong = ScriptLib.GetGroupVariableValue(context, "gear_belong_"..cid)
	--定位玩家序号
	u_index = 0
	...
	result = LF_Enough_Points(context, o_id, player)
	if 1 == result then
		p_o_id = ScriptLib.GetGroupVariableValue(context, "gear_info_"..(evt.param1//1000))
		ScriptLib.PrintContextLog(context, "## -----------------get_variable : gear_info_"..(evt.param1//1000).."->"..p_o_id)
	 	...
	 	--================================================================
	 	buff_cache = ScriptLib.GetGroupVariableValue(context, "buff")
...
function LF_Return_Points(context, index, player)
	if index > 0 then
		...
		new_points = cur_points + define.option_points[index]
		if new_points > define.max_points then
			new_points = define.max_points
		end
...
function fix_gear(context, prev_context, param1, param2, param3)
	for i,v in ipairs(points) do
		cid = ScriptLib.GetGroupVariableValue(context, "gear_info_"..i)
		if cid ~= 0 then
			cid = 1000*i+cid+1
