...
function LF_Initialize_Level()
    firstConfigID = 2450001
    for k,v in ipairs(door) do
	    gadgetConfig = gadgets[v]
	    ...
	    gadgets[v].operatorConfigID = firstConfigID + 1000
	     singleTrigger = { config_id = firstConfigID, name = "PirateShojiStateChange" .. v, event = EventType.EVENT_GADGET_STATE_CHANGE,
...
function action_PirateShojiOption(context,evt)
	...
	if 70360001 == evt.param2 then
		...
		doorConfigID = gadgets[operatorConfigID].chainConfigID
		gadgetState = ScriptLib.GetGadgetStateByConfigId(context, 0, doorConfigID)
...
function action_PirateShojiStateChange(context,evt)
	doorConfigID = evt.param2
	operatorConfigID = gadgets[doorConfigID].operatorConfigID
...
function action_PirateShojiSelectOption(context, evt)
	...
	if 1 == evt.param2 and 70360169 == gadgets[doorConfigID].gadget_id then
		groupID = ScriptLib.GetContextGroupId(context)
		gadgetID = ScriptLib.GetGadgetStateByConfigId(context, 0, doorConfigID)
		if 0 == gadgetID then
			isLock = ScriptLib.GetTeamAbilityFloatValue(context, uid, "_SCENEOBJ_PirateShoji_IsOpen")
			if isLock < 1 then
				-- 提示不能开门
