...
-- 怪物
monsters = {
	{ config_id = 2001, monster_id = 24010101, pos = { x = 0.035, y = 511.859, z = -8.703 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, pose_id = 100 },
	{ config_id = 2002, monster_id = 24020201, pos = { x = 3.903, y = 511.859, z = -7.820 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, pose_id = 100 }
}
...
-- 装置
gadgets = {
	{ config_id = 2021, gadget_id = 70900205, pos = { x = 5.727, y = 510.077, z = 6.490 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_2022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 191, 360, 240617002, 2, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_2023(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240617003, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_2024(context, evt)
	...
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240617001, 1001, {7}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240617002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
